...
export const actions = actionTree(
  { state, mutations },
  {
    ...
    async [_MangaActions.SEARCH_MANGAS]({ commit }, { page, tag }): Promise<void> {
      const mangas: Paginate<Manga> = await this.$axios.$get(`manga/search-mangas?limit=12&page=${page}&tags=${tag}`)
